...
int base( int );
opecode mnemonic2i(char *);
instruction getcode(char *);
...
int base( int l ) /* trace static link l times, where l is the difference */
...
void linearize(instruction c[]){
    ...
    cleararray( (char *)code, sizeof(code) );
    cleararray( (char *)label, sizeof(label)); /* label is the label table */
    if( cx2 == 1 ) return;
